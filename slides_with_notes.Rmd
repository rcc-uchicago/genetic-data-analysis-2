...
+ **download_genotypes.sh:** Script that can be used to download the
  genotype data.

+ **functions.R**: Some R functions used to load the data and create
  plots.
...
First, compute a "realized relatedness" matrix from the genotypes.
...
files containing the results of these two GEMMA commands:

```
tibia.cXX.txt.gz
tibia_lmm.assoc.txt.gz
tibia_lmm.log.txt
```

To use the previously computed GEMMA results, first use gunzip to
uncompress them:
...
(e.g., due to populations tructure or "hidden" relatedness).

+ The q-q plot is commonly used to assess inflation. This test is
  useful as a simple diagnostic.
...
+ LMMs can *overcorrect* inflation. One solution is the
...
gwscan <-
  read.table("../output/tibia_lmm.assoc.txt",
             as.is = "rs",header = TRUE)
head(gwscan)
nrow(gwscan)
```
