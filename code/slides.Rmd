...
library(cowplot) # Optional.
source("functions.R")
p6 <- plot.inflation(gwscan$p_lrt)
print(p6)
```

Let's quit R, and next we will attempt an LMM-based association
analysis, which can reduce inflation due to population structure and
"hidden" relatedness.
...
genotypes. This is actually the most computationally intensive step.
