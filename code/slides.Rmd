...
Let's take a quick look at the GEMMA files:
...
Combine the genotypes into one file. This can be easily done with
the `cat` command, which combines by lines (rows):
...
**Question:** Why didn't we combine the genotypes inside the R
script?
...
common mistake is to save the phenotypes and genotypes in different
orders, which will lead to an incorrect association analysis!*

+ **Question:** Why is it easy to make this mistake in GEMMA?
